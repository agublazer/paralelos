...
const int RMAX = 100; // Los valores del arreglo son enteros entre 0 y RMAX-1
 ...
 * Purpose:    Llenar el arreglo local con nÃºmeros aleatorios
...
/*-------------------------------------------------------------------
 * Function:    Get_args
 * Purpose:     Obtiene y revisa los argumentos de la lÃ­nea de comandos
...
/*-------------------------------------------------------------------
 * Function:   Read_list
 * Purpose:    el proceso 0 lee el arreglo global y lo reparte (scatter)
 ...
 * Purpose:    Junta todas las listas y las imprime
 * Input args: local_A, local_n, my_rank, p, comm
 *
 * Nota: Crea un arreglo interno A para almacenar temporalmente el arreglo global
 */
...
/*-------------------------------------------------------------------
 * Function:    Compare
 * Purpose:     Compara 2 enteros, retornando -1, 0, o 1, si el
 *              primer entero es menor, igual, o mayor al segundo,
 *              respectivamente.  Esta funciÃ³n es requerida por qsort.
 */
int Compare(const void* a_p, const void* b_p) {
   int a = *((int*)a_p);
   int b = *((int*)b_p);
...
/*-------------------------------------------------------------------
 * Function:    Sort
 * Purpose:     Algoritmo odd-even para el ordenamiento de la
 *              lista global.
...
void Sort(int local_A[], int local_n, int my_rank,
   ...
   /* Arreglos temporales para intercambio de informaciÃ³n con los proc. vecinos */
   ...
   if (my_rank % 2 != 0) {
      ...
      if (even_partner == -1) even_partner = MPI_PROC_NULL;  // sin vecino durante la fase
      odd_partner = my_rank + 1;
      if (odd_partner == p) odd_partner = MPI_PROC_NULL;  // sin vecino durante la fase
   } else {
      even_partner = my_rank + 1;
      if (even_partner == p) even_partner = MPI_PROC_NULL;  // sin vecino durante la fase
      odd_partner = my_rank-1;
      if (odd_partner == -1) odd_partner = MPI_PROC_NULL;  // sin vecino durante la fase
   }

   /* Ordenar la lista local usando el algoritmo (serial) QuickSort de la biblioteca */
 ...
 * Purpose:     Ejecuta una fase del algoritmo odd-even
...
/*-------------------------------------------------------------------
 * Function:    Merge_low
 * Purpose:     Coloca los local_n elementos mÃ¡s pequeÃ±os de my_keys
 *              y recv_keys dentro de temp_keys.  Luego copia temp_keys
 *              a my_keys.
...
/*-------------------------------------------------------------------
 * Function:    Merge_high
 * Purpose:     Coloca los local_n elementos mÃ¡s grandes de my_keys
 *              y recv_keys dentro de temp_keys.  Luego copia temp_keys
 *              a my_keys.
 * In args:     local_n, recv_keys
 * In/out args: my_keys
...
/*-------------------------------------------------------------------
 * Esta funciÃ³n sÃ³lo es llamada por el proceso 0
 */
void Print_list(int local_A[], int local_n, int rank) {
   int i;
   printf("*** %d:\n ", rank);
 ...
 * Purpose:    Escribe los arreglos de todos los procesos
 * Input args: all
 * Notes:
